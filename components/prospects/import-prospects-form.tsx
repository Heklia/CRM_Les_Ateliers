...
import { importProspects } from "@/app/prospects/import/actions";
...
const csvTemplate = [
  "entreprise;segment;contact;fonction_contact;telephone;email;ville;code_postal;potentiel_estime;commentaire",
  "Exemple Paysage;Agencements decoratifs;Camille Martin;Dirigeante;0600000000;camille@example.fr;Nantes;44000;15000;Projet decoratif exterieur"
].join("\n");

export function ImportProspectsForm() {
  const [state, formAction] = useFormState(importProspects, initialState);

  return (
    <section className="rounded-lg border border-border bg-surface p-5 shadow-soft">
      <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
        <div>
          <h2 className="text-base font-semibold">Fichier CSV</h2>
          <p className="mt-2 text-sm leading-6 text-muted">
            Importez un fichier CSV separe par point-virgule ou virgule. Les champs obligatoires sont
            entreprise, segment et contact.
          </p>
        ...
        <form action={formAction} className="grid gap-4">
          ...
          <div className="rounded-md border border-border bg-background p-4 text-sm text-muted">
            <p className="font-medium text-foreground">Segments acceptes</p>
            <p className="mt-2">1 ou Agencements decoratifs</p>
            <p>2 ou Structures et mobilier</p>
            <p>3 ou Usinage 3D et prototypage</p>
          </div>
...
function SubmitButton() {
  ...
  return (
    <Button className="w-full md:w-auto md:justify-self-start" disabled={pending} type="submit">
      <Upload size={16} />
      {pending ? "Import en cours..." : "Importer les prospects"}
    </Button>
  );
}
