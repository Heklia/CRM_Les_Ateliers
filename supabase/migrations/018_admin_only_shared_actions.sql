-- Migration 018 - actions partagees modifiables uniquement par les admins.
...
with check (
  public.is_admin()
  and public.can_access_prospect(prospect_id, owner_user_id)
);
...
begin
  if not public.is_admin() then
    raise exception 'Seul un admin peut modifier une fiche action partagee';
  end if;
  ...
  if not public.can_access_prospect(thread_row.prospect_id, thread_row.owner_user_id) then
